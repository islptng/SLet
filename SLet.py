...
# Detect if the environment is Try It Online.
if argv[0] == ".code.tio":
	from warnings import warn
	WarnText = """
=======
TIO environment detected.
Please write your code in the first line in the 'input' section.
After the first line they'll be treated as input.
Save the code as "SLet4.py" on your computer to get better experience.
======="""
	warn(SyntaxWarning(WarnText))
	code = input()
	lamb = Lambda(code)
	lamb.call()
	exit()
...
def help():
	import webbrowser
	webbrowser.open("https://esolangs.org/wiki/SLet/algo")
...
Type "&h" for help, "&v" to see variables, "&r" to reset variables,
and "&c" to clear screen. Press Ctrl+C to quit.""" % (VERSION, LAST_MODIFIED))
while True:
	try:
		code = input("\n=> ")
	...
	if code[0] == "&":
		if code[1:] == "h": help()
		if code[1:] == "v": vars()
		if code[1:] == "r": variables = {}
		if code[1:] == "c": clearscreen()
		if code[1] == " ":
			filename = code[2:]
			try:
				...
				try:
					with open(filename + ".slet", "r") as f:
						code = f.read()
						try:
							execute(code)
						except Exception as e:
							print("ERROR:",e)
				except: print("ERROR: No such file")
	else:
		try:
			execute(code)
		except Exception as e:
